...
export const humanoid = system("humanoid", realm => {
	const cores = new Map<Ecs4.Id, HumanoidCore>()

	return [
		system("core", () => [
			behavior("create core babylon parts")
				.select(...select.core)
				.lifecycle(() => (init, id) => {
					const core = humanoid_core(realm, init)
					cores.set(id, core)
					return {
						tick: () => {},
						end: () => {
			...
			behavior("create viewable parts")
				.select(...select.core, "camera", "third_person_cam_distance")
				.lifecycle(() => (init, id) => {
					const core = cores.get(id)!
					const viewable = humanoid_viewable(realm, {...init, core})
					// viewables.set(id, viewable)
					return {
						tick: () => {},
						end: () => {
							// viewables.delete(id)
			...
			behavior("set gimbal rotations onto transforms")
				.select(...select.core, "gimbal")
				.processor(() => () => (state, id) => {
					const {transform, torusRoot} = cores.get(id)!
			...
			behavior("apply capsule movement, set grounding and airborne_trajectory")
				.select(...select.core, "impetus", "grounding", "airborne_trajectory")
				.processor(() => tick => (state, id) => {
					const {capsule} = cores.get(id)!
			...
			behavior("update position and rotation")
				.select(...select.core, "position", "rotation")
				.lifecycle(_ => (init, id) => {
					const {capsule, transform} = cores.get(id)!
			...
			behavior("apply transform position")
				.select(...select.core, "position", "smoothing")
				.processor(() => () => (state, id) => {
					const {transform} = cores.get(id)!
		...
		// system("viewable", () => {
		// 	const viewables = new Map<Ecs4.Id, HumanoidViewable>()
		// 	return [
		// 		behavior("create viewable parts")
		// 			.select(...select.core, "camera", "third_person_cam_distance")
		// 			.lifecycle(() => (init, id) => {
...
// 	const coreParts = new Map<Ecs4.Id, Parts>()
...
// 			.processor(() => () => (state, id) => {
// 				const {transform, torusRoot} = coreParts.get(id)!
// 				const moddedGimbal = apply_spline_to_gimbal_y(state.gimbal, [.1, .5, .7])
// 				const quaternions = gimbaltool(moddedGimbal).quaternions()
// 				transform.rotationQuaternion = quaternions.horizontal
// 				torusRoot.rotationQuaternion = quaternions.vertical
...
// 			.select("humanoid", "impetus", "grounding", "airborne_trajectory")
// 			.processor(() => tick => (state, id) => {
// 				const {capsule} = coreParts.get(id)!
// 				const {grounded} = capsule.applyMovement(state.impetus)
...
// 			.lifecycle(_ => (init, id) => {
// 				const {capsule, transform} = coreParts.get(id)!
// 				let smoothed_y = init.position[1]
// 				return {
// 					tick(_, state) {
// 						smoothed_y = molasses(
...
// 			.processor(() => () => (state, id) => {
// 				const {transform} = coreParts.get(id)!
// 				transform.position = babylonian.from.vec3(
// 					molasses3d(
// 						state.smoothing,
// 						babylonian.to.vec3(transform.position),
...
// 		system("user controllable humanoid", realm => {
// 			type Parts = ReturnType<typeof create_controllable_humanoid_babylon_parts>
